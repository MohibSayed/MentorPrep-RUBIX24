...
export const updateMentorAvailability = async (req, res, next) => {
  try {
    ...
    let availabilityToUpdate = mentor.availability.find(
      (avail) => avail.date === date
    );

    if (!availabilityToUpdate) {
      // return res.status(404).json({ message: 'Availability not found for the given date' });
      const day = getDayFromDate(date); // You can implement a function to get the day from the date
      ...
      mentor.availability.push(availabilityToUpdate);
      await mentor.save();
